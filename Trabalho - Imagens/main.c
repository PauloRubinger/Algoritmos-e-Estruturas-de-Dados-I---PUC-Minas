...
void leMatriz(char *file, int altura, int largura, int **img) {
	...
	for (int i = 0; i < largura; i++) { // Percorre as linhas da matriz (altura da imagem)
		for (int j = 0; j < altura; j++) { // Percorre as colunas da matriz (largura da imagem)
...
void imprimir_matriz(int altura, int largura, int **img) {
...
void gira180Normal(int altura, int largura, int **img, int **imgDestino) {
	...
	for (int i = 0; i < largura; i++){
		for (int j = 0; j < altura; j++){
			imgDestino[i][j] = img[largura - (i + 1)][j];
		}
	}
...
void gira180Espelho(int altura, int largura, int **img, int **imgDestino) {
	...
	for (int i = 0; i < largura; i++){
		for (int j = 0; j < altura; j++){
			imgDestino[i][j] = img[largura - (i + 1)][altura - (j + 1)];
		}
	}
...
void gira270(int altura, int largura, int **img, int **imgDestino) {
...
int main() {
	...
	float reducao = 0.75;	// controle tonalidade, sendo que quanto menor o valor, mais escuro (maior a redução)
