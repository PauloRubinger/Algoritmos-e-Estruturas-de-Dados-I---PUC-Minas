...
void leAlturaLarguraMaxColor(char *file, int *largura, int *altura, int *maxColor) {
	...
	// As próximas linhas após a primeira mostram a largura, altura e tonalidade máxima da imagem
...
void gira180Normal(int altura, int largura, int **img, int **imgDestino) {
	...
	for (int i = 0; i < altura; i++){
		for (int j = 0; j < largura; j++){
			imgDestino[i][j] = img[altura - (i + 1)][j]; // Troca a primeira linha com a última, a segunda com a penúltima e assim por diante e vice-versa
		}
	}
...
void gira180Espelho(int altura, int largura, int **img, int **imgDestino) {
	...
	for (int i = 0; i < altura; i++){
		for (int j = 0; j < largura; j++){
			imgDestino[i][j] = img[altura - (i + 1)][largura - (j + 1)]; // Troca a primeira linha com a última, a segunda com a penúltima e assim por diante e vice-versa, e também inverte direita com esquerda
		}
	}
...
void gira270Espelho(int altura, int largura, int **img, int **imgDestino) {
	...
	for (int i = 0; i < largura; i++){
		for (int j = 0; j < altura; j++){
			imgDestino[i][j] = img[j][largura - (i + 1)];	// Troca as linhas pelas colunas e espelha as linhas para que a primeira seja a última, a segunda a penúltima e assim por diante e vice-versa
		}
	}
